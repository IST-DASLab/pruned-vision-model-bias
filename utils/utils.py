...
class TrainingProgressTracker(object):
    stats = ['loss', 'acc', 'pred_pos', 'time', 'lr']
    ...
    def val_info(self, epoch_num, val_loss, val_correct, pred_pos):
        ...
        self.tb_logger.add_scalar('val_pred_pos', 1.0 * pred_pos / self.val_size, epoch_num)

        logging.info(f'Epoch [{epoch_num}] Test set: Average loss: {val_loss:.4f}, ' +
                     f'Top1: {val_correct:.0f}/{self.val_size} ' +
                     f'PredPos: {pred_pos:.0f}/{self.val_size} ' +
    ...
    def sparsity_info(self,
        ...
        for sparsity_dict in sparsity_dicts:
            print("sparsity_dicts", sparsity_dicts)
