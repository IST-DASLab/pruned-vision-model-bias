...
def get_macs_sparse(data, device, original_model, multiply_adds=True, ignore_zero=True, display_log=True,
                 ignore_bn=False, ignore_relu=False, ignore_maxpool=False, ignore_bias=False):
    # Taken from DPF code (DPF paper: https://openreview.net/forum?id=SJem8lSFwB)
    # ---------------
...
def get_flops(data, device, model):
    model = get_unwrapped_model(model)
    total_flops, module_flops, module_names = get_macs_sparse(data, device, model, multiply_adds=True, ignore_zero=True,
                                                           display_log=True,
                                                           ignore_bn=False, ignore_relu=False, ignore_maxpool=False,
                                                           ignore_bias=False)
